...
//!
//! Consolidates the spawn + process-group-kill + reap logic a host needs to
//! safely drive the `ato` CLI as a supervised child process.
...
mod tests {
    ...
    fn resolves_binary_via_host() {
        ...
        assert_eq!(
            sup.host().resolve_binary("ato").unwrap(),
            PathBuf::from("/fake/bin/ato")
        );
    }
