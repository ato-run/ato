...
//!
//! Everything above this trait ([`crate::supervisor`]) is generic over it, so
//! adding a new host means "implement `RunnerHost`", not "re-implement
//! supervision". A host knows how to spawn, observe, and tear down processes on
//! its platform and how to resolve the ato-family binaries — nothing about
//! Capsule semantics (that stays in the CLI).
...
/// child) — this is the invariant that keeps orphaned children from surviving a
/// cancelled or exited CLI wrapper.
...
#[derive(Debug, Clone)]
pub enum OutputSink {
    /// Append to a log file. Avoids the pipe-buffer stall a long-running child
    /// hits when nothing drains its pipe.
    LogFile(PathBuf),
...
/// (desktop today; others later). Must be `Send + Sync` so a supervisor can be
/// shared across the host's tasks.
pub trait RunnerHost: Send + Sync {
    /// The concrete supervised-child type this host produces.
    type Child: ManagedChild;

    /// Resolve an ato-family binary (e.g. `"ato"`) to an absolute path on this
    /// host.
