...
impl LocalCasIndex {
    ...
    pub fn open(root_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        std::fs::create_dir_all(&chunks_dir).map_err(|e| {
            CapsuleError::Runtime(format!("failed to create {}: {e}", chunks_dir.display()))
        })?;
    ...
    pub fn put_verified_chunk(&self, chunk_hash: &str, chunk_bytes: &[u8]) -> Result<PathBuf> {
        ...
        atomic_write_file(&full_path, chunk_bytes).map_err(|e| {
            CapsuleError::Runtime(format!("failed to write {}: {e}", full_path.display()))
        })?;
    ...
    pub fn load_chunk_bytes(&self, chunk_hash: &str) -> Result<Option<Vec<u8>>> {
        ...
        let bytes = std::fs::read(&full_path).map_err(|e| {
            CapsuleError::Runtime(format!("failed to read {}: {e}", full_path.display()))
        })?;
    ...
    pub fn build_bloom(&self, fp_rate: Option<f64>) -> Result<AtoBloomFilter> {
        let r = |e: rusqlite::Error| CapsuleError::Runtime(e.to_string());
        let conn = self.connect()?;
        let mut stmt = conn
            .prepare("SELECT chunk_hash, rel_path FROM local_chunks")
            .map_err(r)?;
        let mut rows = stmt.query([]).map_err(r)?;
        let mut hashes = Vec::new();
        let mut stale = Vec::new();

        while let Some(row) = rows.next().map_err(r)? {
            let chunk_hash: String = row.get(0).map_err(r)?;
            let rel_path: String = row.get(1).map_err(r)?;
    ...
    pub fn available_hashes_for_manifest(&self, chunk_hashes: &[String]) -> Result<Vec<String>> {
        ...
        let conn = self.connect()?;
        let mut stmt = conn
            .prepare("SELECT rel_path FROM local_chunks WHERE chunk_hash=?1")
            .map_err(r)?;
    ...
    pub fn chunk_count(&self) -> Result<usize> {
        let conn = self.connect()?;
        let count: i64 = conn
            .query_row("SELECT COUNT(1) FROM local_chunks", [], |row| row.get(0))
            .map_err(|e| CapsuleError::Runtime(e.to_string()))?;
        Ok(count.max(0) as usize)
    }

    fn connect(&self) -> Result<Connection> {
        let r = |e: rusqlite::Error| {
            CapsuleError::Runtime(format!("failed to open {}: {e}", self.db_path.display()))
        };
        let conn = Connection::open(&self.db_path).map_err(r)?;
...
fn normalize_blake3_hash(value: &str) -> Result<String> {
...
fn canonical_hash(value: &str) -> Result<String> {
