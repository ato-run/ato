...
const STAGING_PREFIX: &str = ".payload-staging-";

pub fn unpack_payload_from_capsule_root(capsule_root: &Path, out_dir: &Path) -> Result<()> {
    let cas_provider = CasProvider::from_env();
    unpack_payload_from_capsule_root_with_provider(capsule_root, out_dir, &cas_provider)
}

pub fn unpack_payload_from_manifest(capsule_root: &Path, out_dir: &Path) -> Result<bool> {
...
pub fn unpack_payload_from_capsule_root_with_provider(
    ...
    match cas_provider {
        CasProvider::Enabled(cas) => {
            unpack_payload_from_manifest_file_with_cas(&manifest_path, out_dir, cas.clone())?;
            Ok(())
        }
        CasProvider::Disabled(reason) => {
            CasProvider::log_disabled_once("payload_unpack", reason);
            Err(CapsuleError::Pack(format!(
                "payload restore failed: CAS is disabled ({}). Cannot unpack {}.",
                reason, PAYLOAD_MANIFEST_PATH
            )))
        }
    }
}
...
fn unpack_payload_from_manifest_file_with_cas(
    ...
    let staging = tempfile::Builder::new()
        ...
        .map_err(|e| {
            CapsuleError::Pack(format!(
                "failed to create payload staging directory in {}: {}",
                out_dir.display(),
                e
            ))
        })?;
    ...
    staging.close().map_err(|e| {
        CapsuleError::Pack(format!(
            "failed to cleanup payload staging directory {}: {}",
            out_dir.display(),
            e
        ))
    })?;
...
fn read_and_verify_manifest(manifest_path: &Path) -> Result<PayloadManifest> {
    ...
    let manifest: PayloadManifest = serde_json::from_slice(&manifest_bytes).map_err(|e| {
        CapsuleError::Pack(format!(
            "failed to parse payload manifest {}: {}",
            manifest_path.display(),
            e
        ))
    })?;
    ...
    fn open_next_decoder(&mut self) -> std::io::Result<bool> {
        ...
        let file = fs::File::open(&spec.path).map_err(|e| {
            std::io::Error::new(
                ...
                format!(
                    "failed to open payload chunk {} at {}: {}",
                    spec.raw_hash,
                    spec.path.display(),
                    e
                ),
        ...
        let decoder = zstd::Decoder::new(file).map_err(|e| {
            std::io::Error::new(
                ...
                format!(
                    "failed to create zstd decoder for payload chunk {} at {}: {}",
                    spec.raw_hash,
                    spec.path.display(),
                    e
                ),
    ...
    fn finalize_current_chunk(&mut self) -> std::io::Result<()> {
        ...
        if self.current_raw_read != spec.raw_size as u64 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!(
                    "payload chunk raw size mismatch for {}: expected {} got {}",
        ...
        if computed_hash != spec.raw_hash {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!(
                    "payload chunk hash mismatch for {}: expected {} got {}",
    ...
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        loop {
            ...
            let read = decoder.read(buf).map_err(|e| {
                let prefix = self
                    .current_spec()
                    .map(|spec| format!("payload chunk {} decode failed", spec.raw_hash))
                    .unwrap_or_else(|| "payload chunk decode failed".to_string());
...
mod tests {
    ...
    fn build_manifest(chunks: Vec<ChunkMeta>) -> PayloadManifest {
    ...
    #[test]
    fn test_untar_fails_closed_on_missing_chunk() {
        ...
        let (manifest, hashes) = build_from_tar(&tar_path, &cas, 1024);
        ...
        let err = unpack_payload_from_manifest_file_with_cas(
            &root.path().join(PAYLOAD_MANIFEST_PATH),
            out.path(),
            cas,
        )
        .unwrap_err();
    ...
    #[test]
    fn test_untar_fails_closed_on_corrupted_chunk() {
        let root = tempfile::tempdir().unwrap();
        let out = tempfile::tempdir().unwrap();
        let cas = CasStore::new(root.path().join("cas")).unwrap();
        let tar_path = root.path().join("payload.tar");
        ...
        assert!(
            err.to_string().contains("decode failed")
                ...
                || err.to_string().contains("payload chunk")
    ...
    #[test]
    fn test_untar_is_pure_streaming() {
        let root = tempfile::tempdir().unwrap();
        let out = tempfile::tempdir().unwrap();
        let cas = CasStore::new(root.path().join("cas")).unwrap();
        let tar_path = root.path().join("payload.tar");
        create_large_tar(&tar_path, 100 * 1024 * 1024);
        let (manifest, _) = build_from_tar(&tar_path, &cas, 4 * 1024 * 1024);
        write_manifest(root.path(), &manifest);
    ...
    fn unpack_payload_manifest_false_when_missing_manifest_file() {
        let root = tempfile::tempdir().unwrap();
        let out = tempfile::tempdir().unwrap();
        assert!(!unpack_payload_from_manifest(root.path(), out.path()).unwrap());
    }
}
