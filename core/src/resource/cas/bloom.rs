...
impl AtoBloomFilter {
    ...
    pub fn from_wire(wire: &AtoBloomWire) -> anyhow::Result<Self> {
        let bits = BASE64.decode(wire.bitset_base64.as_bytes())?;
        if bits.is_empty() {
            anyhow::bail!("bloom bitset must not be empty");
        }
        if wire.k_hashes == 0 {
            anyhow::bail!("bloom k_hashes must be greater than zero");
        }
