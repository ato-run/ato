...
pub enum LaunchError {
    ...
    MissingConfig {
        ...
        fields: Vec<ato_ipc::config::ConfigField>,
...
struct ResolveEnvelope {
    ...
    /// See `ato_ipc::ccp::enforce_ccp_compat` for the tolerance rules.
    #[serde(default)]
    schema_version: Option<String>,
    resolution: ResolvePayload,
}

impl ato_ipc::ccp::HasSchemaVersion for ResolveEnvelope {
...
impl ato_ipc::ccp::HasSchemaVersion for SessionStartEnvelope {
    fn schema_version(&self) -> Option<&str> {
        self.schema_version.as_deref()
    }
}
...
impl ato_ipc::ccp::HasSchemaVersion for SessionStopEnvelope {
    fn schema_version(&self) -> Option<&str> {
        self.schema_version.as_deref()
    }
}
...
pub fn stop_capsule_session(session_id: &str) -> Result<bool> {
    ...
    ato_ipc::ccp::enforce_ccp_compat(&stopped, "session_stop")?;
    Ok(stopped.stopped)
}
...
fn resolve_capsule(handle: &str) -> Result<ResolvePayload> {
    ...
    ato_ipc::ccp::enforce_ccp_compat(&envelope, "resolve_handle")?;
...
impl ato_ipc::ccp::HasSchemaVersion for LatestEnvelope {
    fn schema_version(&self) -> Option<&str> {
        self.schema_version.as_deref()
    }
}
...
pub fn fetch_latest_capsule_version(handle: &str) -> Result<Option<String>> {
    ...
    ato_ipc::ccp::enforce_ccp_compat(&envelope, "fetch_latest")?;
...
fn run_session_start_command(
    ...
    ato_ipc::ccp::enforce_ccp_compat(&envelope, "session_start")
...
fn build_launch_session_from_stored(
    ...
    let trust_state = match stored.trust_state {
        ato_ipc::handle::TrustState::Unknown => "unknown",
        ato_ipc::handle::TrustState::Untrusted => "untrusted",
        ato_ipc::handle::TrustState::Trusted => "trusted",
        ato_ipc::handle::TrustState::Promoted => "promoted",
        ato_ipc::handle::TrustState::Local => "local",
    }
    .to_string();
...
mod tests {
    ...
    use ato_ipc::handle::{CapsuleDisplayStrategy, CapsuleRuntimeDescriptor};
