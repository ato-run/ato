...
fn main() {
    ...
    for line in stdin.lock().lines().map_while(Result::ok) {
        ...
        let response = match request.get("method").and_then(|value| value.as_str()) {
            Some("initialize") => {
                serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"protocolVersion":"2025-03-26","capabilities":{"tools":{}},"serverInfo":{"name":"ato-desktop","version":env!("CARGO_PKG_VERSION")}}})
            }
...
fn call_tool(id: serde_json::Value, request: &serde_json::Value) -> serde_json::Value {
    ...
    let command = match params.get("name").and_then(|value| value.as_str()) {
        Some("run_capsule") => ComputationCommand::RunPortable {
            capsule_file: params
                .get("arguments")
                .and_then(|value| value.get("capsule_file"))
                .and_then(|value| value.as_str())
                .unwrap_or_default()
                .to_owned(),
        },
        _ => {
            return serde_json::json!({"jsonrpc":"2.0","id":id,"error":{"code":-32602,"message":"unknown tool"}});
        }
    };
    match ato_desktop::dispatch(&command) {
        Ok(result) => {
            serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"isError":!result.success,"content":[{"type":"text","text":result.output}]}})
        }
        Err(error) => {
            serde_json::json!({"jsonrpc":"2.0","id":id,"error":{"code":-32000,"message":error.to_string()}})
        }
    }
}
