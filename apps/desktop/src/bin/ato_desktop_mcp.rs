    ...
    for line in stdin.lock().lines().map_while(Result::ok) {
        ...
        let response = match request.get("method").and_then(|value| value.as_str()) {
            ...
            Some("tools/list") => serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"tools":[
                {"name":"run_computation","description":"Advance a repository computation","inputSchema":{"type":"object","properties":{"source":{"type":"string"}},"required":["source"]}},
                {"name":"list_runs","description":"List mutable computation cursors","inputSchema":{"type":"object","properties":{}}}
            ]}}),
...
fn call_tool(id: serde_json::Value, request: &serde_json::Value) -> serde_json::Value {
    ...
    let command = match params.get("name").and_then(|value| value.as_str()) {
        Some("run_computation") => ComputationCommand::Run {
            source: params.get("arguments").and_then(|value| value.get("source")).and_then(|value| value.as_str()).unwrap_or(".").to_owned(),
        },
        Some("list_runs") => ComputationCommand::ListRuns,
