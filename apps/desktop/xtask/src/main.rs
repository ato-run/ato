...
fn run_cargo_build(
    ...
    // `apps/desktop/target/...` while ato + nacelle land in the
...
impl WorkspacePaths {
    fn discover() -> Result<Self> {
        ...
        let desktop_root = xtask_root
            .parent()
            .map(Path::to_path_buf)
            .context("xtask crate must live under <repo>/apps/desktop/xtask")?;
...
mod tests {
    ...
    fn workspace_paths_include_netd_manifest() {
        ...
        assert!(
            paths
                .netd_manifest
                .ends_with(Path::new("services/netd/Cargo.toml")),
