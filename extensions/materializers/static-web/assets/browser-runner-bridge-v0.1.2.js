const BROWSER_RUNNER_BRIDGE_VERSION = "0.1.2";
...
class BrowserRunnerBridge {
  ...
  receive(connectionId, value) {
    ...
    this.applyChain = this.applyChain.then(
      () => this.apply(connection, operation, receivedAt),
      () => this.apply(connection, operation, receivedAt)
    );
  ...
  async apply(connection, operation, receivedAt) {
    ...
    const ack = {
      ...
      controller_session_id: authority.controller_session_id,
      logical_action_id: operation.logical_action_id ?? operation.operation_id,
      kind: operation.kind,
      sent_at: operation.sent_at ?? receivedAt,
    ...
    this.broadcast({
      ...
      controller_session_id: ack.controller_session_id,
      logical_action_id: ack.logical_action_id,
      kind: ack.kind,
      sent_at: ack.sent_at,
...
class AtoBrowserDocumentBridge {
  ...
  async startMedia(payload) {
    requireExactKeys(payload, ["generation", "ice_servers", "peer_id", "seat"]);
    ...
    this.runner.serve(
      new DataChannelRunnerConnection(
        `remote_${signal.peer_id}_${signal.generation}`,
        channel
      )
    );
  ...
  async answerMedia(payload) {
    requireExactKeys(payload, ["generation", "peer_id", "seat", "sdp"]);
    const signal = parseMediaSignal(payload);
    this.requireRun(signal);
  ...
  async addMediaIce(payload) {
    requireExactKeys(payload, ["candidate", "generation", "peer_id", "seat"]);
    const signal = parseMediaSignal(payload);
    this.requireRun(signal);
  ...
  requireRun(signal) {
    if (signal.seat !== this.identity.seat) {
      throw new Error("media_identity_mismatch");
    }
  }
...
function parseMediaSignal(value) {
  if (typeof value.peer_id !== "string" || !ID.test(value.peer_id)) {
    throw new Error("invalid_media");
  }
  if (!Number.isSafeInteger(value.generation) || value.generation <= 0 || value.seat !== 0 && value.seat !== 1) {
    throw new Error("invalid_media");
  }
  return {
...
function sameMediaSignal(left, right) {
  return left.peer_id === right.peer_id && left.generation === right.generation && left.seat === right.seat;
}
