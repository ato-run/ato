...
impl ReplaySequence {
    pub fn load(
        ...
        validate_replay_policies(&sequence.records, &sequence.required_adapters, adapters)?;
        Ok(sequence)
    }
...
impl Materializer for ReplayMaterializer {
    ...
    fn encode(
        ...
        validate_replay_policies(context.records, &required_adapters, context.adapters)?;
        let anchor = context
            .records
            .first()
