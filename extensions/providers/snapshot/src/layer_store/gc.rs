...
impl GcReport {
    ...
    pub fn eviction_log_lines(&self) -> Vec<String> {
        let mut lines: Vec<String> = self
            .evicted
            .iter()
            .map(|e| format!("layer store gc: evicted {} ({} bytes)", e.hash, e.bytes))
            .collect();
        lines.push(format!(
            "layer store gc: kept {}, evicted {}, reclaimed {} bytes",
            self.kept,
            self.evicted.len(),
...
mod tests {
    ...
    fn unreferenced_chunks_are_reclaimed() {
        ...
        let reader = crate::layer_store::LazyBlobReader::new(&store, &live);
