...
use snapshot::layer_store::{CasStore, ContentHash};
...
///
/// Reuses the snapshot provider's layer-store reachability sweep.
...
fn discard_failed_attempt_scratch(jobdir: &Path, pinned: &HashSet<ContentHash>) {
    ...
    match snapshot::layer_store::gc::collect_garbage(&store, &[], pinned) {
