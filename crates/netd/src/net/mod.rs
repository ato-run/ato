...
//! derivation) live in `ato_protocol::net`; this module re-exports them under
...
//! wire surface single-sources in `ato-protocol` and the runtime code lives
//! next to the daemon that runs it.

pub mod client;

/// Control-plane surface: wire DTOs from `ato_protocol::net::control`, plus
...
/// `ato_protocol::net::resolver`, plus the `hickory`-backed backends.
pub mod resolver {
    pub use ato_protocol::net::resolver::*;
...
pub use ato_protocol::net::receipt;
pub use ato_protocol::net::stable_origin;
