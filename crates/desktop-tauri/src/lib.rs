...
//! `ato-pwa` Desktop build (a local-asset WebView) and, over time, exposes a
//! typed `DesktopHost` command/event adapter over the abstracted `runner` crate
//! (which supervises the `ato` CLI — the sole owner of capsule execution).
//!
