...
/// confirmation for this privileged action.
///
/// No-op if a registration is already in progress, or if this device is already
/// registered (re-authorizing would require an explicit unregister first; that
/// is a follow-up). In both cases the current status is returned without
/// launching a second browser device-flow.
...
/// Best-effort terminate the runner agent process **and its descendants** (the
/// `ato runner serve` wrapper plus any sandboxed runs it spawned).
///
/// On Unix the serve child is its own process-group leader
/// (`spawn_runner_serve` sets `process_group(0)`), so a negative-pid `SIGKILL`
/// reaps the whole group. Windows has no process groups, so `taskkill /T /F`
/// walks the process tree to the same effect — without this the foreground-only
...
/// Foreground-only teardown: kill the serve agent (and its descendants) on
/// Desktop shutdown so the Connected Runner does not linger online after the
/// Desktop exits. Safe to call without a GPUI context (invoked from
