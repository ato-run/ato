//! `SURFACE-TIMING` — supervision-stage timing telemetry, the host-side
//! analogue of the CLI `PHASE-TIMING` infrastructure (RFC:
//! SURFACE_MATERIALIZATION, Phase 0 / PR 3).
//!
//! Lives in `runner` because it times execution/launch supervision stages
//! (`resolve_subprocess`, `session_start_subprocess`, `build_launch_session`,
//! …) rather than any GUI rendering — it is self-contained (only `std::time`
//! and `std::env`) and emits gated stderr lines. The Desktop shell re-exports
//! it from `crate::surface_timing`, so its existing call sites are unchanged.
//!
...
pub fn emit_total(total_ms: u64, result_kind: &str, extras: &SurfaceExtras) {
    if !enabled() {
        return;
    }
    let mut line = format!(
