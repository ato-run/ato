...
fn main() {
    ...
    let size_mib: u64 = arg(&["--size-mib"]).and_then(|s| s.parse().ok()).unwrap_or(1024);

    // Source: a local checkout, or materialize a server-resolved owner/repo/commit.
    let src: PathBuf = if let Some(dir) = arg(&["--source", "-s"]) {
        PathBuf::from(dir)
    } else {
        let owner = arg(&["--owner"]).unwrap_or_else(|| fail("args", "--source or --owner/--repo/--commit required".into()));
    ...
    let toml = std::fs::read_to_string(&toml_path).unwrap_or_else(|e| fail("source", format!("read {}: {e}", toml_path.display())));
