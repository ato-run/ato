...
pub fn execute(plan: &ManifestData) -> Result<i32> {
    ...
    let status = child.wait().map_err(|e| {
        CapsuleError::ProcessStart(format!(
            "Failed waiting for OCI engine {}: {e}",
            engine_binary(&engine)
        ))
    })?;
...
async fn run_oci_with_metrics(container_id: String, image_hash: String) -> Result<()> {
    let docker = connect_docker().map_err(|e| {
        CapsuleError::ContainerEngine(format!("Failed to connect to Docker daemon: {e}"))
    })?;
...
fn connect_docker() -> Result<Docker> {
    if let Some(host) = resolve_docker_host() {
        if let Some(path) = host.strip_prefix("unix://") {
            return Docker::connect_with_local(path, 120, API_DEFAULT_VERSION).map_err(|e| {
                CapsuleError::ContainerEngine(format!("Failed to connect via unix socket: {e}"))
            });
        }

        if let Some(path) = host.strip_prefix("npipe://") {
            return Docker::connect_with_local(path, 120, API_DEFAULT_VERSION).map_err(|e| {
                CapsuleError::ContainerEngine(format!("Failed to connect via named pipe: {e}"))
            });
        }

        if let Some(addr) = host.strip_prefix("tcp://") {
            let http = format!("http://{}", addr);
            return Docker::connect_with_http(&http, 120, API_DEFAULT_VERSION).map_err(|e| {
                CapsuleError::ContainerEngine(format!("Failed to connect via http: {e}"))
            });
        }

        if host.starts_with("http://") {
            return Docker::connect_with_http(&host, 120, API_DEFAULT_VERSION).map_err(|e| {
                CapsuleError::ContainerEngine(format!("Failed to connect via http: {e}"))
            });
        }
    }
    ...
    Docker::connect_with_local_defaults().map_err(|e| {
        CapsuleError::ContainerEngine(format!("Failed to connect with local defaults: {e}"))
    })
}
...
fn resolve_image(plan: &ManifestData) -> Result<String> {
    ...
    Err(CapsuleError::Config(
        "OCI runtime selected but no image specified".into(),
    ))
}
