...
pub fn execute(plan: &ManifestData) -> Result<i32> {
    ...
    if which::which("wasmtime").is_err() {
        return Err(CapsuleError::NotFound("wasmtime".into()));
    }
    ...
    let status = cmd
        ...
        .status()
        .map_err(|e| CapsuleError::ProcessStart(format!("Failed to run wasmtime: {e}")))?;
...
fn resolve_component(plan: &ManifestData) -> Result<String> {
    ...
    Err(CapsuleError::Config(
        "Wasm runtime selected but no component path found".into(),
    ))
}
