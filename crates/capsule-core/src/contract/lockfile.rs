...
use crate::common::paths::{
    manifest_dir, workspace_artifacts_dir, workspace_derived_dir, workspace_tmp_dir,
};
...
fn lockfile_has_required_platform_coverage(
    ...
    // Deno: filtered to platforms where Deno publishes a binary (not all platforms are supported).
    // `supported_deno_platforms` removes targets for which no Deno artifact filename can be derived.
    ...
    // Python: filtered to platforms where the selected Python version has a prebuilt binary.
    // Coverage requirement depends on the locked Python version (some older versions lack ARM builds).
    if let Some(targets) = runtime_targets
    ...
    // Node.js: requires all requested platforms. Node publishes binaries for all major targets.
    ...
    // uv: filtered to platforms where uv publishes a binary. uv coverage mirrors Python's own
    // supported-platform list, so not every Rust target is guaranteed to have a uv binary.
    ...
    // pnpm: requires all platforms. pnpm is distributed as a JS bundle via npm and runs
    // on any platform that has Node.js, so no platform-specific binary filter is needed.
    if let Some(targets) = lockfile
        .tools
        .as_ref()
...
struct LockfileConfigContext<'a> {
    manifest_raw: &'a toml::Value,
    manifest_dir: &'a Path,
    reporter: &'a Arc<dyn CapsuleReporter + 'static>,
    timings: bool,
    runtime_platforms: &'a [RuntimePlatform],
}

struct LockfileState<'a> {
    runtimes: &'a mut RuntimeSection,
    tools: &'a mut ToolSection,
    targets: &'a mut HashMap<String, TargetEntry>,
}

async fn configure_python_lockfile(
    ctx: &LockfileConfigContext<'_>,
    state: &mut LockfileState<'_>,
    target_key: &str,
    required_runtime_version: Option<&str>,
) -> Result<()> {
    let version = required_runtime_version
        .map(str::to_string)
        .or_else(|| read_runtime_version(ctx.manifest_raw))
        .unwrap_or_else(|| read_language_version(ctx.manifest_raw, "python", "3.11"));
    let step_started = Instant::now();
    let python_lockfile =
        generate_uv_lock(ctx.manifest_dir, ctx.manifest_raw, ctx.reporter.clone()).await?;
    maybe_report_timing(
        ctx.reporter,
        ctx.timings,
    ...
    let step_started = Instant::now();
    let runtime =
        resolve_python_runtime(&version, ctx.runtime_platforms, ctx.reporter.clone()).await?;
    maybe_report_timing(
        ctx.reporter,
        ctx.timings,
    ...
    .await?;
    state.runtimes.python = Some(runtime);
    ...
    if python_lockfile.is_some() {
        ...
        let target_entry = state.targets.entry(target_key.to_string()).or_default();
        ...
        let step_started = Instant::now();
        state.tools.uv =
            Some(resolve_uv_tool_targets(ctx.runtime_platforms, ctx.reporter.clone()).await?);
        maybe_report_timing(
            ctx.reporter,
            ctx.timings,
...
async fn configure_node_lockfile(
    ...
    let version = required_runtime_version
        .map(str::to_string)
        .or_else(|| read_runtime_version(ctx.manifest_raw))
        .unwrap_or_else(|| read_language_version(ctx.manifest_raw, "node", "20"));
    let step_started = Instant::now();
    let node_lockfile = generate_node_lockfile(
        ctx.manifest_dir,
        ctx.manifest_raw,
        &version,
        ctx.reporter.clone(),
    ...
    let step_started = Instant::now();
    let runtime =
        resolve_node_runtime(&version, ctx.runtime_platforms, ctx.reporter.clone()).await?;
    maybe_report_timing(
        ctx.reporter,
        ctx.timings,
    ...
    .await?;
    state.runtimes.node = Some(runtime);

    if state.runtimes.deno.is_none() {
        let deno_version = read_language_version(ctx.manifest_raw, "deno", "2.6.8");
        let step_started = Instant::now();
        let deno_runtime =
            resolve_deno_runtime(&deno_version, ctx.runtime_platforms, ctx.reporter.clone())
                .await?;
        maybe_report_timing(
            ctx.reporter,
            ctx.timings,
        ...
        .await?;
        state.runtimes.deno = Some(deno_runtime);
    }
...
async fn configure_deno_lockfile(
    ...
    let version = required_runtime_version
        .map(str::to_string)
        .or_else(|| read_runtime_version(ctx.manifest_raw))
        .unwrap_or_else(|| read_language_version(ctx.manifest_raw, "deno", "2.6.8"));
    let step_started = Instant::now();
    let runtime =
        resolve_deno_runtime(&version, ctx.runtime_platforms, ctx.reporter.clone()).await?;
    maybe_report_timing(
        ctx.reporter,
        ctx.timings,
        "lockfile.resolve_deno_runtime",
    ...
    if let Some(node_version) = runtime_tools.get("node") {
        let step_started = Instant::now();
        let runtime =
            resolve_node_runtime(node_version, ctx.runtime_platforms, ctx.reporter.clone()).await?;
        maybe_report_timing(
            ctx.reporter,
            ctx.timings,
            "lockfile.resolve_node_runtime",
    ...
    if let Some(python_version) = runtime_tools.get("python") {
        let step_started = Instant::now();
        let runtime =
            resolve_python_runtime(python_version, ctx.runtime_platforms, ctx.reporter.clone())
                .await?;
        maybe_report_timing(
            ctx.reporter,
            ctx.timings,
...
/// Generate an `ensure_*_runtime_if_missing` async helper.
///
/// Each generated function checks whether the named `RuntimeSection` field is `None`
/// and, if so, calls the corresponding `resolve_*_runtime` function to populate it.
/// Adding a new runtime requires only one invocation of this macro.
///
/// Note: `RuntimeSection` fields are serialized as JSON keys in `capsule.lock.json`,
/// so the field name must never be changed without a versioned migration.
macro_rules! ensure_runtime_if_missing {
    ($fn_name:ident, $field:ident, $resolve_fn:ident) => {
        async fn $fn_name(
            runtimes: &mut RuntimeSection,
            version: &str,
            runtime_platforms: &[RuntimePlatform],
            ...
            if runtimes.$field.is_none() {
                runtimes.$field =
                    Some($resolve_fn(version, runtime_platforms, reporter.clone()).await?);
            }
            Ok(())
        }
    };
...
/// Returns a manifest target key for the current platform (e.g. `"macos-arm64"`).
///
/// Uses `"arm64"` for aarch64 on all platforms — this matches the manifest key convention
/// where Apple/macOS-style short names are preferred over Rust target triple names.
/// Compare with `platform_triple()` which returns full Rust target triples.
fn platform_target_key() -> Result<String> {
    ...
    // Intentionally "arm64" (not "aarch64") to match capsule manifest key conventions.
...
/// Returns the full Rust target triple for the current platform (e.g. `"aarch64-apple-darwin"`).
///
/// Uses `"aarch64"` (not `"arm64"`) because this value is passed directly to toolchain
/// infrastructure that expects standard Rust target triple names.
/// Compare with `platform_target_key()` which uses the shorter manifest key format.
