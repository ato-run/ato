...
pub fn execution_descriptor_from_manifest_parts(
    ...
    let (selected_target, runtime_model) = if is_v03_native {
        let target = manifest_routing::resolve_v03_target(&manifest, target_label)?;
        let model = manifest_routing::synthesize_runtime_model_from_v03(&manifest, &target)?;
        (target, model)
    } else {
        let target = manifest_routing::resolve_target_label(&manifest, target_label)?;
        let model = manifest_routing::synthesize_runtime_model_from_manifest(&manifest, &target)?;
        (target, model)
    };
...
impl ExecutionDescriptor {
    ...
    pub fn typed_manifest(&self) -> Result<CapsuleManifest> {
        self.compat_manifest
            ...
            .ok_or_else(|| CapsuleError::Config("compat manifest bridge is unavailable".into()))
    }
    ...
    pub fn build_lifecycle_build(&self) -> Option<String> {
        self.compat_str(&["targets", &self.selected_target, "build"])
            .or_else(|| self.compat_str(&["targets", &self.selected_target, "build_command"]))
            .or_else(|| self.compat_str(&["build", "lifecycle", "build"]))
    }

    pub fn install_command_string(&self) -> Option<String> {
        self.compat_str(&["targets", &self.selected_target, "install"])
            .or_else(|| self.compat_str(&["targets", &self.selected_target, "install_command"]))
            .or_else(|| self.compat_str(&["install"]))
            .or_else(|| self.compat_str(&["install_command"]))
    }

    pub fn prestart_command_string(&self) -> Option<String> {
        self.compat_str(&["targets", &self.selected_target, "prestart"])
...
"#,
        ...
        .unwrap();
        assert_eq!(
            super::manifest_routing::resolve_v03_target(&manifest, Some("custom")).unwrap(),
            "custom"
        );
    }
...
"#,
        ...
        .unwrap();
        let target = super::manifest_routing::resolve_v03_target(&manifest, None).unwrap();
...
"#,
    ...
    fn v03_split_runtime_selector() {
        assert_eq!(
            super::manifest_routing::split_v03_runtime("web/static"),
