...
/// Accept at most five minutes of control-plane idle grace. This staging
/// backport serves web previews only, where the legacy idle field is a no-op;
/// validating the field keeps its ACK contract aligned with the full runner.
...
async fn handle_restore_snapshot_lease(
    ...
    // this one serving-start instant. `parse_restore_snapshot_command` guarantees a
    // preview lease carried `max_duration_secs`, so it is always Some here. This
    // staging branch predates pixel-stream input telemetry, so `idle_timeout_secs`
    // remains the existing no-op for its web-only preview surface.
    ...
    if cmd.is_preview {
        println!(
            "⏳ restore lease {lease_id}: preview lane (max_duration_secs={:?}, runner_cap_secs={}, idle_timeout_secs={:?} [v0 stub])",
