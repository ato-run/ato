...
fn worker_sim_produces_remote_layer_consumed_by_lookup() -> Result<()> {
    ...
    cli::artifact_build_worker_sim::run_fixture_file_backed_artifact_build_worker(
        &fixture_dir(),
        &worker_workspace,
        &remote,
    )?;
...
fn export_output_layer_to_remote(
    ...
    cli::phase_materializer_remote::export_recorded_build_output_layer_to_remote_mirror(
        remote_root,
        workspace,
    )
}
