...
pub fn write_capsule_lock(workspace_root: &Path, fixture_name: &str) {
    ...
    let manifest: capsule::types::CapsuleManifest =
        toml::from_str(&manifest_text).expect("failed to parse manifest");
    let hash = capsule::packers::payload::compute_manifest_hash_without_signatures(&manifest)
...
pub fn prepare_fixture_workspace(fixture_name: &str) -> (TempDir, PathBuf) {
    ...
    if lock_path.exists() {
        ...
        let manifest: capsule::types::CapsuleManifest =
            toml::from_str(&manifest_text).expect("failed to parse manifest");
        let rendered = capsule::lockfile::render_lockfile_for_manifest(&lock_path, &manifest)
