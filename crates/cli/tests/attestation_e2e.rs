...
use cli::dependency_materializer::DepDerivationKeyV1;
use cli::dependency_materializer::freeze::freeze_dep_tree;
use cli::dependency_materializer::{
...
fn freeze_emits_attestation_when_attestation_key_env_is_set() {
    ...
    let key_path = env.path().join("attestation-key.json");
    fs::write(
        &key_path,
        serde_json::to_vec_pretty(&key.to_stored()).unwrap(),
    )
    .unwrap();
    let _key_env = EnvVarGuard::set("ATO_ATTESTATION_KEY", &key_path);
    let _builder = EnvVarGuard::set("ATO_ATTESTATION_BUILDER_ID", "ato-cli@e2e-test");
    ...
    let outcome =
        cli::provider_cache::freeze_after_install(&deps, &derivation_hash, "npm").unwrap();
