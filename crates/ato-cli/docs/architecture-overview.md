...
- services: top-level [services] の graph 検証と startup/readiness 協調
...
- cleanup: attempt 単位の cleanup journal、cleanup report、PipelineAttemptError
...
run 側も、仕様上およびアーキテクチャ上は Install -> Prepare -> Build -> Verify -> DryRun -> Execute の順で読むのが正確です。つまり Install は consumer pipeline の外側にある前処理ではなく、application が意味論を持つ最初の phase です。ここでの Install の意味は producer と別物ではなく、どちらも artifact をクリーンインストールして後続 phase が「すでに展開済みの具体的ディレクトリ」を相手にできる状態を作ることです。違うのは用途ではなく行き先だけで、consumer ではその installed environment が実行ルートになり、producer では dry-run 検証ルートになります。CLI 側の commands は入力解決、adapter 構築、request 正規化を担い、application::engine::install::support などの補助ロジックは ConsumerRunPipeline が進める Install phase を支える部品として位置づけられます。実装上も ConsumerRunPipeline の standard start は Install であり、manifest 自動生成が必要だった場合は Install 成功後にそこで正常停止します。
...
cleanup モジュールは、phase 実行中に生成した一時ディレクトリや補助プロセスなどの後始末を attempt 単位で束ねる役割を持ちます。HourglassPipeline と ProducerPipeline は phase ごとに PipelineAttemptContext を受け渡し、失敗時には cleanup journal を unwind してから PipelineAttemptError として返します。これにより cleanup 失敗を主エラーと分離したまま diagnostics 層へ渡せます。

#### services

src/application/services は、top-level [services] 用の application 層です。現状の主要モジュールは次のとおりです。

- graph: ServiceGraphPlan による DAG 検証、topological order、layer 構築
- coordinator: ServicePhaseCoordinator / ServicePhaseRuntime による service startup/readiness 協調

この層の役割は、runtime adapter の中に散らばりやすい service graph policy を application 側に寄せることです。現在の coordinator は「start は layer 内で直列、readiness 待機は layer ごとに並列」という実装になっており、web_services と orchestrator の両方がこの契約を共有しています。つまり、低レベルの spawn や OCI API 呼び出しは adapters 側に残しつつ、依存順序と barrier の意味論は application::services が担います。
...
- diagnostics: エラー分類、cleanup report / manifest suggestion 付き JSON envelope、終了コード対応
- progressive: ロゴや段階的表示
- terminal: TUI / ターミナル向け表現

application::ports::OutputPort を実装する具体物はこの周辺に置かれます。

diagnostics は単なる文字列整形ではなく、AtoExecutionError と PipelineAttemptError を受けて classification、cleanup_status、cleanup_actions、manifest_suggestion を JSON / human output に投影する境界になっています。cleanup の実行結果や manifest 修正候補を機械可読なまま保持するのが現在の設計です。
...
現在の services 実行では、runtime executor が application::services の coordinator を利用します。web_services は同期 API のまま async coordinator を bridge し、orchestrator は local / OCI 混在の service graph を同じ coordinator 契約で起動します。どちらも、service graph 検証は ServiceGraphPlan、startup/readiness の barrier は ServicePhaseCoordinator、実際の spawn / inspect / shutdown は runtime adapter、という責務分割で読めます。
...
ただし現在は sidecar cleanup だけでなく、application::pipeline::cleanup の shared journal を通じて子プロセス停止や一時ディレクトリ削除も同じ失敗処理モデルへ寄せられています。lib.rs の sidecar 後始末はその一部であり、cleanup の主語全体は application 側に移っています。
...
4. application::pipeline の consumer 側 phase 語彙に沿って Install -> Prepare -> Build -> Verify -> DryRun -> Execute を進める
5. Install phase では SourcePort / TargetPort と install::support まわりの補助ロジックを使って、artifact 取得、展開先解決、local manifest 準備、実行対象の確定を行う
6. missing / invalid manifest の補修で新しい capsule.toml を生成した場合は、Install 完了時点で正常終了し、Prepare 以降へは進まない
7. watch 経路も同じ Install 解決を先に通り、その後に監視対象 manifest へ正規化される
8. adapters::runtime が実ランタイム差分を吸収し、adapters::output::reporters が結果を出力する

ここで重要なのは、CLI が Install を先回りして完了させるのではなく、Install を含む consumer flow 全体に必要な依存性を組み立てて application に渡す点です。これにより、consumer / producer の両方で Install を一貫して phase として扱えます。

また、phase 実行中に生成された資源は PipelineAttemptContext に登録され、失敗時には cleanup を実行したうえで diagnostics へ渡されます。run の理解には、phase order と cleanup/reporting が同じ application::pipeline の一部として結びついている点も重要です。
