...
use super::cache::{ArtifactCache, CachedRuntime};
...
impl ArtifactManager {
    ...
    pub fn resolve_cas_uri(&self, uri: &str) -> Result<PathBuf, ArtifactError> {
        ...
        let Some(hash) = uri.strip_prefix(CAS_PREFIX) else {
    ...
    async fn download_and_install(
        ...
        let temp_file_path = temp_dir.join(format!("download_{}.zip", random_suffix()));
