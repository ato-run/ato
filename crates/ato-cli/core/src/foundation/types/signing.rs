...
pub fn parse_developer_key(value: &str) -> Result<[u8; 32]> {
    ...
    if value.starts_with("did:key:") {
        return identity::did_to_public_key(value)
            .map_err(|e| CapsuleError::Crypto(format!("failed to parse did:key: {e}")));
    }
...
impl StoredKey {
    ...
    pub fn read(path: &Path) -> Result<Self> {
        let raw = fs::read_to_string(path).map_err(|e| {
            CapsuleError::Runtime(format!("failed to read key file {}: {e}", path.display()))
        })?;
        let stored: StoredKey = serde_json::from_str(&raw).map_err(|e| {
            CapsuleError::Runtime(format!("failed to parse key file {}: {e}", path.display()))
        })?;
        Ok(stored)
    }
    ...
    pub fn to_signing_key(&self) -> Result<SigningKey> {
        if self.key_type.as_str() != "ed25519" {
            return Err(CapsuleError::Crypto(format!(
                "unsupported key_type {}; expected ed25519",
                self.key_type
            )));
        }
        let secret_bytes = BASE64
            .decode(&self.secret_key)
            .map_err(|err| CapsuleError::Crypto(format!("failed to decode secret key: {err}")))?;
        ...
        if public_encoded != self.public_key {
            return Err(CapsuleError::Crypto(
                "public key mismatch between stored public and derived secret".into(),
            ));
        }
        Ok(signing_key)
    }
    ...
    pub fn did(&self) -> Result<String> {
        let public_bytes = BASE64
            ...
            .map_err(|err| CapsuleError::Crypto(format!("failed to decode public key: {err}")))?;
...
pub fn write_signature_file(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| {
            CapsuleError::Runtime(format!(
                "failed to create signature directory {}: {e}",
                parent.display()
            ))
        })?;
    }
...
pub fn read_signature_file(path: &Path) -> Result<SignatureFile> {
    let data = fs::read(path).map_err(|e| {
        CapsuleError::Runtime(format!("failed to read signature {}: {e}", path.display()))
    })?;
    if data.len() < 1 + 1 + 32 + 64 + 2 {
        return Err(CapsuleError::Crypto("signature file too short".into()));
    }
    let version = data[0];
    let key_type = data[1];
    let mut offset = 2;
    ...
    if data.len() < offset + metadata_len {
        return Err(CapsuleError::Crypto(
            "signature metadata length out of bounds".into(),
        ));
    }
...
pub fn ensure_signature_matches_manifest(sig: &SignatureFile, developer_key: &str) -> Result<()> {
    ...
    if sig.public_key != manifest_key {
        return Err(CapsuleError::Crypto(
            "signature public key does not match manifest developer_key".into(),
        ));
    }
    Ok(())
}
...
pub fn verify_signature_file(sig: &SignatureFile, message: &[u8]) -> Result<()> {
    ...
    let verifying = VerifyingKey::from_bytes(&sig.public_key)
        .map_err(|_| CapsuleError::Crypto("failed to parse signature public key".into()))?;
    verifying
        .verify(message, &sig.signature)
        .map_err(|_| CapsuleError::Crypto("signature verification failed".into()))
}
...
pub fn sign_capsule_artifact(
    ...
    let canonical = serde_jcs::to_vec(&pre_image)
        .map_err(|e| CapsuleError::Crypto(format!("JCS canonicalization failed: {e}")))?;
...
pub fn verify_capsule_artifact_signature(
    ...
    if sig.manifest_hash != expected_manifest_hash {
        return Err(CapsuleError::Crypto(format!(
            ...
            expected_manifest_hash, sig.manifest_hash
        )));
    }
    ...
    if sig.payload_hash != expected_payload_hash {
        return Err(CapsuleError::Crypto(format!(
            ...
            expected_payload_hash, sig.payload_hash
        )));
    }

    let key_bytes = parse_developer_key(&sig.key_id)?;
    let verifying_key = VerifyingKey::from_bytes(&key_bytes)
        .map_err(|_| CapsuleError::Crypto("failed to parse key_id as Ed25519 public key".into()))?;
    ...
    let sig_bytes = BASE64
        ...
        .map_err(|e| CapsuleError::Crypto(format!("failed to decode signature base64: {e}")))?;
    if sig_bytes.len() != 64 {
        return Err(CapsuleError::Crypto(format!(
