...
fn build_accepts_windows_single_file_native_artifacts() -> Result<()> {
    ...
    let result =
        build_native_artifact_with_strip(&plan, Some(&artifact_path), |_path| Ok(()), None, false)?;
...
fn build_native_artifact_preserves_source_and_payload_executable_mode() -> Result<()> {
    ...
    let result =
        build_native_artifact_with_strip(&plan, Some(&artifact_path), |_app| Ok(()), None, false)?;
