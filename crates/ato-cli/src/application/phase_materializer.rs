...
pub(crate) fn capture_build_outputs(
    ...
    let blob_hash = freeze_build_output_tree(&capture_root, &materialization_key)?;
...
fn freeze_build_output_tree(tree_root: &Path, materialization_key: &str) -> Result<String> {
    let _lock = DerivationLock::acquire(materialization_key)?;
