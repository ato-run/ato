...
- For CLI, desktop, MCP, and manual smoke verification, use a fresh hermetic env root on every run by default; do not reuse the same `ATO_HOME` across retries unless you are explicitly debugging state carry-over.
- In this repo, `scripts/ato-test-shell.sh` and `tests/manual/config.sh` allocate a unique env root per invocation by default. Reuse must be an explicit opt-in via `ATO_TEST_REUSE_ENV_ROOT=1` plus `ATO_TEST_ENV_ROOT=<existing-root>`.
...
- The pushed tag triggers `apps/ato-cli/.github/workflows/release.yml` to publish artifacts and create/update the GitHub Release.
...
- `Release / Release PR` with command `release` requires `RELEASE_PLZ_TOKEN` (PAT); if that secret is missing, the step fails before pushing any tag and `release.yml` will not trigger.
