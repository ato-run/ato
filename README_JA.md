...
- source project の native delivery metadata は `capsule.toml` に書く必要があります。`ato.delivery.toml` は authored input としてはもう受け付けず、artifact 内部の互換 metadata としてのみ stage されます。
...
native target が command mode（`entrypoint = "sh"` と `cmd = [...]`）のときは、現時点では明示的な delivery metadata が必要です。対応する source 上の置き場所は `capsule.toml` 内の `[artifact]` + `[finalize]` だけです。source 側の `ato.delivery.toml` は fail-closed で拒否します。片方だけの inline metadata も fail-closed で拒否されます。
...
- 現在の方針では、source project は `capsule.toml` のみを使い、`ato.delivery.toml` は artifact 内部の互換 metadata としてのみ残します。
...
1. **現在**: native delivery の authored source manifest は `capsule.toml` のみをサポートする。
