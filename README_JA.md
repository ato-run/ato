...
`ato` は1行のインストーラーで導入できます：

```bash
curl -fsSL https://ato.run/install.sh | sh
```

または、[GitHub Releases ページ](https://github.com/ato-run/ato-cli/releases/latest)からお使いのプラットフォーム向けバイナリをダウンロードして `PATH` に置いてください。

以下はソースからビルドして動かすワークフロー（コントリビューター / CI 向け）です。
...
```bash
# 試す・実行する
ato run https://ato.run/s/myproject@r1 [--entry <name>] [--env-file <path>] [--prompt-env]
...
# プロセス管理
ato ps
ato stop --id <capsule-id> | --name <name> [--all] [--force]
...
人間用の曖昧な設定ではなく、機械可読な `ato.lock.json` が設定の「唯一の正本（SSOT）」です。このファイルがあれば、マシンの差異を問わず全く同じ環境の復元が保証されます。

旧バージョンの Ato で作成した `capsule.toml` を持つプロジェクトは、互換パスを通じてそのまま動作します——手動変更は不要です。そのプロジェクト内で `ato init` を実行すると、永続的な `ato.lock.json` が生成され、標準フローへ移行できます。
...
ネイティブアプリの配信・統合機能です。TauriやElectronなどのプロジェクト構造からエントリポイント（macOS では `.app` など）を自動推論し、ネイティブ配信に必要な設定をすべて `ato.lock.json` へ自律的に解決・記録します（別途設定ファイルを用意する必要はありません）。

ネイティブ配信メタデータを含んだ成果物を利用者が `ato install` すると、OSのアプリケーション領域へ**投影（プロジェクション）**され、通常のネイティブアプリとして利用可能になります。投影は成果物が対応している場合にデフォルトで有効です。`--no-project` を渡すとスキップ、`--project` を渡すと強制的に有効化できます。
