...
`ato` は `ato.lock.json` を canonical なコマンド入力として解釈しつつ、互換入力と source-only の bootstrap フローも受け付けて、実行・配布・インストールを行うメタCLIです。
...
- `ato.lock.json` が存在する場合はそれが正本です。
- compatibility / bootstrap 入力は移行前から引き続き利用できます。
- `capsule.lock.json` は legacy な互換データで、単独のコマンド入力としては受け付けません。
- canonical と互換入力が同時にある場合は `ato.lock.json` が優先され、互換入力は advisory 扱いです。
- `ato inspect lock` / `ato run` / `ato build` はこの優先順位で解決します。

## Native Delivery（実験的）

- 基本の product surface は引き続き `ato build` / `ato publish` / `ato install` です。
- 現在の Tauri darwin/arm64 PoC では、native delivery metadata は project manifest に書きます。ただし `ato.lock.json` が存在する場合はそちらが正本です。既定 target に `driver = "native"` と `.app` の `entrypoint` があれば native build を検出します。
- source project の native delivery metadata は project manifest に書く必要があります。`ato.delivery.toml` は authored input としてはもう受け付けず、artifact 内部の互換 metadata としてのみ stage されます。
...
native delivery authoring 向けの canonical な project manifest contract は次です。
...
native target が command mode（`entrypoint = "sh"` と `cmd = [...]`）のときは、現時点では明示的な delivery metadata が必要です。対応する source 上の置き場所は project manifest 内の `[artifact]` + `[finalize]` だけです。source 側の `ato.delivery.toml` は fail-closed で拒否します。片方だけの inline metadata も fail-closed で拒否されます。

### 互換 sidecar と artifact metadata flow

- `ato.delivery.toml` は source project で使う manifest ではありません。local finalize/install/project のために artifact 内へ stage される内部 metadata としてのみ残ります。
- build は、source project が canonical input しか持っていない場合でも、常に `ato.delivery.toml` を artifact payload に stage します。これにより source tree がなくても local finalize/install で artifact 自身が自己記述的になります。
...
- 現在の方針では、source project は canonical input を使い、`ato.delivery.toml` は artifact 内部の互換 metadata としてのみ残します。
...
- install JSON: `install_kind`, `launchable`, `local_derivation`, `projection`
  ...
  - `launchable.path` は caller が run に使うべき path です
...
1. **現在**: native delivery の authored source manifest は canonical input のみをサポートする。
2. **現在**: `ato` は local finalize/install/project のために `ato.delivery.toml` を artifact 内部 metadata として stage し続ける。
...
- カスタム/私設レジストリ（上記以外の `--registry`）:
  ...
  `--artifact` はローカルの project manifest がなくても単体で publish できます。
...
- `CAPSULE_WATCH_DEBOUNCE_MS`: `run --watch` のデバウンス間隔（ms, default: `300`）
