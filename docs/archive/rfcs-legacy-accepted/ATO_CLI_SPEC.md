...
> **SSOT 注記 (2026-08):** 本ファイルは旧版のコピーです。現行の正は
> `crates/cli/docs/rfcs/accepted/ATO_CLI_SPEC.md` に一本化されています。
> 差分がある場合はそちらを優先してください。また web ベースの共有フロー
> （`ato encap` / `ato decap` と `ato.run/s/<id>` リンク）は廃止されました。
> 共有はローカルファイル（`share.spec.json` / `share.lock.json`）のみで行います。
...
CLI の front-door は `run`, `workspace share`, `workspace setup` に固定する。物語は `Try -> Keep -> Share` とし、他コマンドは互換・高度機能として help 上の優先度を下げる。
  ...
  - local `share.spec.json` / `share.lock.json` を `run` 入力として受理する（web 経由の share URL は受け付けない）。
...
- `ato workspace setup <target> --into <dir> [--plan] [--dev]`
...
> 注記 (2026-08): 旧 `encap`/`decap` の web アップロード/フェッチ（`/v1/shares`, `ato.run/s/<id>` リンク）は廃止された。共有はローカルファイル（`share.spec.json` / `share.lock.json`）のみで行う。本表の `WorkspaceCapture` は upload のない local capture のみを指す。
