...
└─ nacelle (interactive: true)
     ...
     │
     └─ Sandbox Enforcer:
          macOS: sandbox-exec -f profile.sb /bin/zsh
...
│  macOS: sandbox-exec + .sb profile (PTY 許可追加)           │
...
| リスク | 重大度 | 緩和策 |
|---|---|---|
| nono Issue #450: macOS PTY stdin ブロック | High | sandbox-exec ラッパーのプロファイルで検証、問題発生時は dev mode fallback |
| evaluate_script() のスループット制限 | Medium | 16ms バッチ + 1MB バッファ上限で実用十分（インタラクティブシェル） |
| sandbox-exec (Seatbelt) は deprecated API | Low | macOS 15 で動作確認済み。Apple が代替 API を提供するまで使用継続 |
