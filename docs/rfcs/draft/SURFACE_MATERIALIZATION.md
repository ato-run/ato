...
- **(Phase 2A 前提) `Surface close ≠ Session stop`**: Phase 1 の実測で
  ...
  RFC patch が必要 → 別 RFC に切り出し済**:
  
  - 仕様: [`SURFACE_CLOSE_SEMANTICS.md`](./SURFACE_CLOSE_SEMANTICS.md)
    (Draft v0.1) — close = surface op / stop = session op、TTL 5 分、
    explicit Stop UI (context menu + command palette + provisional
    shortcut)、graceful + non-blocking eviction、app quit で全 stop。
  - 実装着手の前提: 上記 RFC の §10.1 acceptance criteria を満たす PR。
  - Phase 2A 本体は §8 の表に従い、close 時の retain 対象を
    "session-only → session + hidden WebView" に拡張する形で接続。
