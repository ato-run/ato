...
### P0: Spec Closure Gate (docs only) — ✅ closed 2026-05-04

**Status**: closed by RFC v1.6 (`docs/rfcs/accepted/CAPSULE_DEPENDENCY_CONTRACTS.md`).

**決めたこと** (RFC §4.4 / §5.3):
- template namespace v1 grammar: 8 種を予約。未定義 key は parser error。escape は v1 で未提供。`{{socket}}` は予約のみで lock fail-closed (`unix_socket = "auto"` 実装と一緒に v1.x で正式予約)
- evaluation order を仕様化: lock 時 = `params` / `env`(parameters 経由) / `identity_exports` / hash 計算、orchestration 直前 = `credentials` / `env`(credentials 経由) / `host` / `port` / `state.dir` / `deps.*.runtime_exports`
- v0.3 legacy `dependencies = "<file>"` (string、language-pkg file ref) と新 RFC の `[dependencies.<X>]` (table) の衝突解決: v0.3 normalizer が string form を `[targets.<default>] dependencies = "<file>"` に fold

**Acceptance** (満たした):
- accepted RFC §4.4 / §5.3 に v1 subset と migration policy が normative として明記
- §13 Open Questions から該当項目を closed 欄へ移動
