...
| **PR 3** | Guest-agent **supervisor mode** (exec/restart workload with composed env, bound-ready after restart); builder placeholder-verify + scrub-before-seal; placeholder sentinel in L4 scan; redaction regression tests | `extensions/providers/snapshot/guest-agent`, `crates/snapshot`, `tools/snapshot-builder` | binding-lease.md's named supervisor mode; L4 scanner |
...
| **PR 7** | v1.2 fixtures (below) + API E2E; L7 multi-binding live E2E folded in | `tools/snapshot-builder/fixtures/compat/`, scripts | v1 compat suite (steps 3–5 of the v1 plan) |
