    ...
    if let Some(cargo_toml) = inputs.cargo_toml.as_ref() {
        ...
        if !has_web_framework
            && matches!(
                ...
                super::super::detect::ProjectType::Rust
            )
        {
            frameworks.push(FrameworkMatch {
                name: "Rust binary",
...
mod tests {
  ...
  "dependencies": {
    ...
    "express": "^4.0.0"
        ...
        .unwrap();
        let detected = super::super::detect::detect_project(tmp.path()).unwrap();
        let info = super::super::recipe::project_info_from_detection(&detected).unwrap();
